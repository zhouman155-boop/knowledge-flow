...
- **分类不是自由生成**：系统内部使用预先定义好的分类表，AI 只能从固定词表中选择 `topic / dimension / content_form`，不能自行发明近义分类名
- **建议先准备好分类框架**：更稳妥的做法是先设计好领域、子领域和内容形式，再让 AI 去匹配；如果一开始让 AI 自由发挥，后期很容易出现同义分类、边界重叠和粒度不一致
