...
━━━ 第二步：分类——以"方便找回"为唯一标准 ━━━
不要固定层数，根据内容自行判断最合适的粒度：

判断原则：
  · 想象用户一个月后想找这篇内容，他会先想到什么关键词？那就是 topic
  · 同一 topic 下可能有多种细分，哪个维度最能区分这篇内容的"用途"？那就是 dimension
  · topic 不要太宽（不要什么都叫"技术"）也不要太窄（不要把一篇文章的标题当 topic）

示例（灵活变化，不固定）：
  一篇工具推荐文 → topic=AI工具, dimension=效率提升
  一篇 Prompt 教程 → topic=提示词工程, dimension=结构化写法
  一篇创业方法论 → topic=创业思维, dimension=早期验证
  一篇健身计划 → topic=健身, dimension=力量训练计划

同类内容要归入同一 topic，让知识库越刷越聚合，不要每篇文章都新建主题。
