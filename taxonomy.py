...
DOMAINS: dict[str, dict] = {
    "AI与大模型": {
        ...
        "boundary": (
            ...
            "Agent 子领域判定：\n"
            "  · 在讲 Agent 整体怎么转（循环、状态机、流式、运行模式）→ Agent循环与运行模式\n"
            "  · 在讲工具怎么注册、执行、调度、截断 → Agent工具系统\n"
            "  · 在讲从原型到产品的工程决策（启动、选型、模块、规模）→ Agent工程实践\n"
            "  · 在讲记忆、上下文压缩、窗口管理 → Agent记忆与上下文\n"
            "  · 在讲安全防护、权限、沙箱 → Agent安全与权限\n"
            "  · 在讲多个 Agent 如何协作 → 多Agent协作"
        ),
    },
    "软件工程": {
        "subdomains": [
...
CONTENT_FORMS: dict[str, str] = {
    ...
    "产品拆解": "围绕一个具体产品/框架做全面介绍或深度拆解，标题须含产品名",
}
